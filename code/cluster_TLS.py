...
from utils import load_pickle, save_pickle, sort_labels, save_image
...
def cluster_and_save(
    ...
    if prefix is not None:
        ...
        plot_label_masks_new(labels, prefix+'masks/')
...
def upscale_image(original_image, scale_factor):
    original_height, original_width = original_image.shape
    new_height = original_height * scale_factor
    new_width = original_width * scale_factor
    upscaled_image = np.zeros((new_height,new_width)).astype(np.float32)
    for new_row in range(new_height):
        for new_col in range(new_width):
            original_row = int(new_row / scale_factor)
            original_col = int(new_col / scale_factor)
            upscaled_pixel = original_image[original_row][original_col]
            upscaled_image[new_row][new_col] = upscaled_pixel.copy()
